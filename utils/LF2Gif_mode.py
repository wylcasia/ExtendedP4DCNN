...
def opts_parser():
    ...
    parser.add_argument(
        '-S', '--save_name', type=str, default=None, dest='save_name',
    ...
    parser.add_argument(
        '-P', '--prefix', type=str, default='View', dest='prefix',
        help='Prefix of sub-image name : (default: %(default)s)')
    parser.add_argument(
        '-F', '--factor', type=int, default=3, dest='factor',
        help='Upsampling Factor: (default: %(default)s)')
    parser.add_argument(
        '-M', '--mode', type=str, default='res', dest='mode',
...
def getNameTupleInOrder(path, length, prefix, ext, factor=3, mode='res'):
    images = []
    # First Loop
    if mode == 'res':
        for n in range(length**2):
            ...
            if col_n == 0:
                ...
                if row_n % 2 == 0:
                    ...
                    col = length - col_n + 1
            images.append(os.path.join(path, prefix+'_{}_{}.'.format(row, col)+ext))
        # Second Loop
        for n in range(length**2):
            col_n = (n+1) // length
            row_n = (n+1) % length
        ...
        for row in range(0, length, factor):
            for col in range(0, length, factor):
                if row // factor % 2 == 1:
                    col = length - 1 - col
                images.append(os.path.join(path, prefix+'_{}_{}.'.format(r=row+1, c=col+1)+ext))
    else:
        raise IOError('No such mode ->%s enabled' % mode)
...
if __name__ == '__main__':
    ...
    images = getNameTupleInOrder(path, length, prefix, ext, factor, mode)
    ...
    if save_name is None:
        gifName = os.path.join('./GIF', '%dx%d_f%d_%s.gif' % (length, length, factor, mode))
    else:
        gifName = os.path.join('./GIF', save_name)
    for image in images:
