...
import numpy as np
from P4DCNN_model import p4dcnn_3L, p4dcnn_6L
...
def opts_parser():
    ...
    usage = "Extended Pseudo 4DCNN Test"
    ...
    parser.add_argument('--dilation_rate', type=int, default=1, dest='dilation_rate',
                        help='Dilation Rate: (default: %(default)d)')
...
def test_Pseudo_4DCNN(
    ...
    if path is not None:
        ...
        if save_path is None:
            ...
            save_path = path+'_{}/'.format(model_config)
    ...
    if model_path.find('3L') != -1:
        p4dcnn_model = p4dcnn_3L
    elif model_path.find('6L') !=-1:
        p4dcnn_model = p4dcnn_6L
    else:
        raise NotImplementedError('No such P4DCNN model supplied.')
    ...
    for indx, scene in enumerate(sceneNameTuple):
        ...
        if save_results:
            ...
            Sparse_save_path = os.path.join(save_path, scene + '_Sparse')
            ...
            if os.path.isdir(Sparse_save_path):
                del_files(Sparse_save_path, 'png')
                log.info('Input Sparse View path %s exists, delete all .png files' % Sparse_save_path)
            else:
                os.mkdir(Sparse_save_path)
        ...
        log.info('...Predicting')
        tic = time.time()
        out_dense_lf = model.predict_on_batch(input_sparse_lf[np.newaxis, :, :, :, :, np.newaxis])[0]
        out_dense_lf = out_dense_lf[:g_u, :g_v, :g_h, :g_w, :]
        ...
        for s_n in range(s_res):
            for t_n in range(t_res):
                ...
                view_img = np.clip(out_dense_lf[s_n, t_n, :, :, 0], gt_img.min(), gt_img.max())
                ...
                if save_results:
                    ...
                    if s_n % factor == 0 and t_n % factor == 0:
                        SparseName = os.path.join(Sparse_save_path, 'View_'+str(s_n+1)+'_'+str(t_n+1)+'.png')
                        io.imsave(SparseName, gt_out_img)
